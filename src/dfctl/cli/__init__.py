...
try:
    REPO: Repo = Repo(CONFIG_PATH_DOTS)
except InvalidGitRepositoryError:
    raise FileExistsError(
        f"Git repo dont exists in '{CONFIG_PATH_DOTS}'\nPlease create it or clone an existing one to start using dfctl"
    )
...
parser.add_argument(
    "--noconfirm",
    action="store_true",
    help=f"skip confirmation prompts                ({CONFIG_NOCONFIRM})",
)
parser.add_argument(
    "--autopull",
    action="store_false",
    help=f"auto pull before any local-repo action   ({CONFIG_AUTOPULL})",
)
parser.add_argument(
    "--autopush",
    action="store_false",
    help=f"auto push after any local-repo action    ({CONFIG_AUTOPUSH})",
)
    ...
    def __enter__(self):
        if self.mode:
            groups = get_target_groups(
                str(self.target), CONFIG_PATH_DOTS, self.mode, self.invert_notfind
            )
        else:
            raise ValueError("specify the mode")
        ...
        if (
            Confirm.ask(f"{self.ask}{self.ask_end}")
